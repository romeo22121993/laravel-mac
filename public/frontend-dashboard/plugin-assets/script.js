jQuery(document).ready(function ($) {
    ...
    */
    ...
    /**
     * Function for Custom Sign Up Page
     *
     */
    function sign_up_custom_page() {

        if ( $(".sign_up_custom_page").length > 0 ) {

            let html = $(".discount_block").html();
            $("#additional_fields_rcp").after(html);
            $(".discount_block").remove();

            let subscription_input = $(".form_shortcode.sign_up_form_inner").find("#subscription_plan");

            $("#rcp_subscription_fieldset").show();
            let arrayDiscounts = {
                'empty': 25,
                'XYPNAI/O': 26,
                'SANCAI/O': 28,
            ...
            let discountCodeBegin = 'empty';

            // Apply discount code functionality
            $(document).on("click", "#custom_rcp_apply_discount", function (e) {
                e.preventDefault();
                $("#loader").show();
                $(this).parents("form").find("#rcp_submit").addClass('disabled_btn');

                let discountCode = $("#custom_rcp_discount_code").val().trim();
                discountCode = !discountCode ? discountCodeBegin : discountCode

                let discountMembership = !arrayDiscounts[discountCode] ? arrayDiscounts[discountCodeBegin] : arrayDiscounts[discountCode]

                let subscriptionLabel = !arrayDiscounts[discountCode] ? discountCodeBegin : discountCode;
                subscriptionLabel = ( subscriptionLabel == 'empty' ) ? 'Standart' : discountCode;
                subscriptionLabel = ( subscriptionLabel == 'SANCAI/O' ) ? 'Sanctuary' : discountCode;

                if ( !arrayDiscounts[discountCode] ) {
                    $(".rcp_custom_discount_invalid").show();
                    $(".rcp_custom_discount_valid").hide();
                    $("#loader").hide();
                }
                else {
                    $(".rcp_custom_discount_valid").show();
                    $(".rcp_custom_discount_invalid").hide();

                    $("#loader").hide();
                }

                $(this).parents("form").find("#rcp_submit").removeClass('disabled_btn');

                // $(".rcp_subscription_level.rcp_subscription_level_"+discountMembership).find("#rcp_subscription_level_"+discountMembership).prop('checked', 'checked')
                $(".rcp_subscription_level.rcp_subscription_level_"+discountMembership).find("#rcp_subscription_level_"+discountMembership).click();
                // setTimeout( sign_up_messages, 4000 );

                $(".form_shortcode.sign_up_form_inner").attr('data-subscription_plan', subscriptionLabel );
                $("form#custom_signup_form").find("#subscription_plan").val(subscriptionLabel);
            });

            $(document).on("click", "#custom_signup_form_before_submit", function (e) {
                e.preventDefault();
				if ( !$('#privacyPolicy').is(":checked") ) {
					$('.privacy_block').append('<p class="rcp_error privacy_block_error"><span>Please read and agree to the Privacy Policy.</span></p>')
				}else {
					$('.privacy_block_error').remove();
				}

                let subscriptionLabel = $(".form_shortcode.sign_up_form_inner").attr('data-subscription_plan');

                let email = $('#rcp_user_email').val(),
                    company = $('#rcp_user_company').val(),
                    fname = $('#rcp_user_first').val(),
                    lname = $('#rcp_user_last').val();

                $('#custom_signup_form input[name=subscription_plan]').val(subscriptionLabel);

                $('#custom_signup_form input[name=email]').val(email);
                $('#custom_signup_form input[name=company]').val(company);
                $('#custom_signup_form input[name=name]').val(fname);
                $('#custom_signup_form input[name=lastname]').val(lname);
                $('#custom_signup_form .contact_submit').trigger('click');
                $('#rcp_registration_form #rcp_submit').trigger('click');
            });
        }
    }

    /**
     * Function for Custom Sign Up Changing Texts Page
     *
     */
    function sign_up_messages() {

        $(document).on('click', '#custom_rcp_apply_discount', function () {
            let discountCode = $("#custom_rcp_discount_code").val();

            if(discountCode !== 'CIONFree' && discountCode !== 'CIONFREE' ) {
            	let textBottom = '';
				if ( ( discountCode === 'XYPNAI/O' ) || ( discountCode === 'SANCAI/O' )  ) {
					textBottom = 'Monthly Access Fee: Get Three (3) months for free and $60.00 Per Month for a Minimum Initial Term of Six (6) Months. Cancel any time after nine (9) months.';
				} else if(discountCode === 'CIONFree' || discountCode === 'CIONFREE' ) {
					textBottom = 'Welcome to Advisor I/O! There\'s no monthly access fee for members of our CION family.';
				}else {
					textBottom = ' Monthly Access Fee: $74.99 Per Month for a Minimum Initial Term of Six (6) Months. Cancel any time after six (6) months.';
				}

				let total_price_html = '<div class="col-lg-12 d-flex justify-content-start align-items-center flex-row">\n' +
                    '                       <label>'+textBottom+'</label>\n' +
                    '                   </div>';

                let len = $("#rcp_gateway_extra_fields .flex-row.justify-content-start.align-items-center label").length;
                setTimeout(function () {
					if (len < 1) {
						$("#rcp_gateway_extra_fields").removeClass('col-12 one-card pxy-20 mt-3 mb-1').addClass( "col-lg-8 offset-lg-2 col-sm-12 case-studies text-center" );
						$("#rcp_gateway_extra_fields").append(total_price_html);
            ...
            if ( ( discountCode === 'XYPNAI/O' ) || ( discountCode === 'SANCAI/O' ) ) {
                $('#rcp_registration_form .case-studies .bigger').text('Monthly Access Fee: Get Three (3) months for free and $60.00 Per Month for a Minimum Initial Term of Six (6) Months. Cancel any time after nine (9) months.');
                $('#rcp_registration_form .rcp_gateway_stripe_fields .flex-row label').text('Monthly Access Fee: Get Three (3) months for free and $60.00 Per Month for a Minimum Initial Term of Six (6) Months. Cancel any time after nine (9) months.');
            } else if(discountCode === 'CIONFree' || discountCode === 'CIONFREE' ) {
                $('#rcp_registration_form .case-studies .bigger').text('Welcome to Advisor I/O! There\'s no monthly access fee for members of our CION family.');
                $('#rcp_registration_form .rcp_gateway_stripe_fields .flex-row label').text('Welcome to Advisor I/O! There\'s no monthly access fee for members of our CION family.');
            }else {
                $('#rcp_registration_form .case-studies .bigger').text(' Monthly Access Fee: $74.99 Per Month for a Minimum Initial Term of Six (6) Months. Cancel any time after six (6) months.');
                $('#rcp_registration_form .rcp_gateway_stripe_fields .flex-row label').text(' Monthly Access Fee: $74.99 Per Month for a Minimum Initial Term of Six (6) Months. Cancel any time after six (6) months.');
            }
    ...
    /**
     * Function of submitting custom sign up form
     *
     */
    function custom_signup_forms() {

        $("#custom_signup_form").on('submit', function (e) {
            e.preventDefault();
            var info = {
                data: $(this).serialize(),
                action: 'custom_signup_action'
            };
            ...
            $.post({
                ...
                data: info,
                success: function (data) {
                    console.log('success submit of custom sign up form');
                },
                error: function (data) {
                    console.log('error');
                },
    ...
    custom_signup_forms();
    sign_up_messages();
    setTimeout( sign_up_page, 100);
    setTimeout( sign_up_custom_page, 100);
