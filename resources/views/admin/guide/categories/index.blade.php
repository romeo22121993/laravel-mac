                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="template-demo" style="margin: 20px 0;">
                            <a href="{{ route('wpadmin.guides.categories.add') }}">
                                ...
                                <tbody>
                                    ...
                                    @foreach( $categories as $category )
                                        <tr>
                                            <td> {{ $i++ }} </td>
                                            <td>
                                                <a href="{{ route('wpadmin.guides.by.categories', $category->id) }}">{{ $category->title }}</a>
                                            </td>
                                            <td> {{ $category->slug }}</td>
                                            <td>{{ $category->guides->count() }}</td>
                                            <td>
                                                <a href="{{ route('wpadmin.guides.categories.edit',  $category->id) }}" class="btn btn-info">Edit</a>
                                                <a href="{{ route('wpadmin.guides.categories.delete', $category->id) }}" onclick="return confirm('Are you sure to delete')" class="btn btn-danger">Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
